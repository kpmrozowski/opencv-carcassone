...
std::vector<cv::Mat> getSquareImages(const cv::Mat &canvas,
                                     const std::vector<Square> &squares) {
  std::vector<cv::Mat> squareImages;
  for (auto square : squares) {
    std::vector<int> y_range =
        std::vector<int>({square.NW.y, square.NE.y, square.SE.y, square.SW.y});
    std::vector<int> x_range =
        std::vector<int>({square.NW.x, square.NE.x, square.SE.x, square.SW.x});
    ...
    // std::cout << "canvas.rows = " << canvas.rows << "canvas.cols = " <<
    // canvas.cols << std::endl; std::cout << "minX = " << *minX << ", minY = "
    // << *minY << ", maxX = " << *maxX << ", maxY = " << *maxY << std::endl;
    // std::cout << "roi.width = " << roi.width << "roi.height = " << roi.height
    // << std::endl;
  }
  return squareImages;
}
...
cv::Mat detect_liness(const char *filename) {
  ...
  while (true) {
    ...
    if (false)
      ...
      Canny(src, dst, canny_treshold1, canny_treshold2, 3, true);
    ...
    */
    ...
    HoughLinesP(dst, linesPCoords, 2, CV_PI / 180, 100, 70,
                30);  // runs the actual detection
  ...
  // std::endl; std::vector<Square> filteredSquares =
  // filteredLines.getSquares(); std::cout << "filteredSquares.size = " <<
  // filteredSquares.size() << std::endl;
