 ...
 public:
  std::vector<float> hog(cv::Mat img) {
    ...
    cv::HOGDescriptor hog(
        cv::Size(templ_gray.cols / cellSize.width * cellSize.width,
                 templ_gray.rows / cellSize.height * cellSize.height),
        cv::Size(blockSize.height * cellSize.height,
                 blockSize.width * cellSize.width),
        cv::Size(cellSize.height, cellSize.width), cellSize, nbins);
    // gives --> winSize [32 x 16],  blockSize [16 x 16],  blockStride [8 x 8],
    // cellSize [8 x 8]

    // compute the descriptor of the car
    hog.compute(templ_gray, hog_vector,
                cv::Size(cellSize.height, cellSize.width), cv::Size(0, 0));
  ...
  std::pair<std::string, int> classifyHog(cv::Mat detected_square) {
    ...
    for (const auto& fname :
         twm::hough::get_filenames("../../../../images/tiles")) {
      std::cout << "./" << fname << '\n';
      const char* fname_cstr = fname.c_str();
      cv::Mat tile =
          imread(cv::samples::findFile(fname_cstr), cv::IMREAD_COLOR);
      for (int rotation = 0; rotation < 4; ++rotation) {
        int detected_square_width = detected_square.cols;
        int detected_square_height = detected_square.rows;

        cv::Mat tile_ref;
        cv::resize(tile, tile_ref,
                   cv::Size(static_cast<std::size_t>(detected_square_width),
                            static_cast<std::size_t>(detected_square_height)));
